"""The same behavioural contract the AWS side asserts, against the Azure port.

Kept deliberately parallel to secure-container-pipeline/tests/test_api.py so a
reviewer can diff the two and see that the API is genuinely identical and only
the cloud underneath changed.
"""
import importlib
...
def test_health_never_touches_the_backend(monkeypatch):
    ...
    main = importlib.import_module("main")
    monkeypatch.setattr(main, "_notes", lambda: FakeContainer(unreachable=True))
...
def test_request_id_is_echoed_from_container_apps_header(client):
    r = client.get("/health", headers={"Request-Id": "abc-123"})
